...
export class CloudinaryProvider implements IStorageProvider {
  ...
  constructor() {
    ...
    if (!process.env.CLOUDINARY_CLOUD_NAME || !process.env.CLOUDINARY_API_KEY || !process.env.CLOUDINARY_API_SECRET) {
      this.logger.warn('Cloudinary credentials not configured. Please check your .env file.');
    } else {
  ...
  async uploadFile(file: any, folder: string): Promise<UploadResult> {
    try {
      this.logger.log(`Uploading to Cloudinary: ${file.originalname} to folder ${folder}`);

      // Convertir el buffer a base64 data URI para Cloudinary
      const base64File = `data:${file.mimetype};base64,${file.buffer.toString('base64')}`;

      // Subir a Cloudinary
      const uploadResult: UploadApiResponse = await cloudinary.uploader.upload(base64File, {
        folder: folder,
  ...
  async deleteFile(publicId: string): Promise<void> {
    try {
      ...
      if (result.result === 'ok') {
        ...
        this.logger.warn(`Could not delete file from Cloudinary: ${publicId}, result: ${result.result}`);
      }
    } catch (error) {
